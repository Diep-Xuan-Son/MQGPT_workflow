...
class Google(ComponentBase, ABC):
    ...
    def __call__(self, inputs, outputs, state):
        ...
        for i, out in enumerate(outputs):
            ...
            try:
                client = GoogleSearch(
                    {"engine": "google", "q": state[inputs[i]], "api_key": self._param.api_key, "gl": self._param.country,
                     "hl": self._param.language, "num": self._param.top_n})
                google_res = ['<a href="' + i["link"] + '">' + i["title"] + '</a>    ' + i["snippet"] for i in
...
if __name__=="__main__":
    param = GoogleParam()
    google = Google(0,param)
    output = google(["question"], ["output"], {"question": "yolov8"})
    print(output)